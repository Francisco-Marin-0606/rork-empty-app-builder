...
// VIDEO_SOURCE se usa desde la prop uri
...
export default function VideoPlayerWithComments({ 
  ...
  const { pauseAuraMusic } = usePauseAuraMusic(initialIsPlaying);
  const [showCommentsModal, setShowCommentsModal] = useState<boolean>(false);
  const [topAreaHeight, setTopAreaHeight] = useState<number>(screenHeight);
  // Eliminamos el estado local isPlaying para evitar conflictos
  ...
  useEffect(() => {
    if (prevIsPlaying.current !== initialIsPlaying) {
      showPlayPauseIcon();
      prevIsPlaying.current = initialIsPlaying;
    }
  }, [initialIsPlaying, showPlayPauseIcon]);

  useEffect(() => {
    console.log('🔄 [VideoPlayerWithComments] totalCommentsCount cambió:', totalCommentsCount);
  ...
  const onStatusUpdate = useCallback((s: AVPlaybackStatus) => {
    ...
    if ('isLoaded' in s && s.isLoaded) {
      // No actualizamos el estado local isPlaying, solo usamos la prop externa
      setIsLoading(false);
      setIsBuffering(s.isBuffering ?? false);
      
      // Call onProgress if provided
      if (onProgress && s.durationMillis && s.positionMillis) {
        onProgress({
  ...
  const handleSkipVideo = async () => {
    try {
      ...
      const v = videoRef.current;
      await v?.pauseAsync?.();
      // No modificamos el estado local isPlaying, solo pausamos el video
    } catch (skipError) {
  ...
  // Helper para verificar si el video está reproduciéndose
  const isVideoPlaying = playbackStatus && 'isLoaded' in playbackStatus && (playbackStatus as any).isPlaying;
      ...
      >
        <Animated.View style={[styles.videoAnimatedContainer, { height: videoHeight }]}> 
          <Video
            ...
            shouldPlay={initialIsPlaying} // Usamos initialIsPlaying de la prop
      ...
      <View style={[styles.controlsRow, Platform.OS === 'android' ? { bottom: 72 } : null]}>
        ...
        >
          ...
          <Text style={styles.chipText} numberOfLines={1} ellipsizeMode="tail">
            {(() => {
              // Usar totalCommentsCount directamente en lugar de commentsCount
              const displayCount = totalCommentsCount > 140 ? '140+' : totalCommentsCount;
              console.log('📊 [VideoPlayerWithComments] Mostrando en botón:', displayCount, '(totalCommentsCount:', totalCommentsCount, ', commentsCount:', commentsCount, ')');
              return displayCount;
            })()}
          </Text>
        </TouchableOpacity>
